...
func InitLogSink()(err error){
	...
	if client, err = mongo.Connect(context.TODO(),
		G_config.MongodbUri, clientopt.ConnectTimeout(time.Duration(G_config.MongodbConnectTimeout) * time.Millisecond));err!=nil{
		return
	}
	...
	for {
		select{
			...
			case timeoutBatch=<-logSink.autoCommitChan: // 过期的批次
				...
				// 每次插入需要等待mongodb的一次请求往返,耗时可能因为网络慢耗费比较长的时间
